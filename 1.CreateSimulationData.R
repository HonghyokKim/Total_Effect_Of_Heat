...
## Unit here is ppb, but this is simulated.
dat$O3_base <- 31 +
  dat$o3_month_effect +
  dat$o3_noise
...
dat$o3_temp25_increment_ppb[dat$HW == 0 & dat$temp >= 25] <-
 2 * (dat$temp[dat$HW == 0 & dat$temp >= 25] - 25)
...
dat$o3_HW_increment_ppb[dat$HW == 1] <- 2


############################################################
## 4.4. HW x high-temperature ozone increment
############################################################

## This component applies only when:
##   HW == 1 and temp >= 25 C
##
## It represents additional ozone formation under heat-wave
## conditions at high temperature.
dat$o3_HW_temp25_increment_ppb <- 0

dat$o3_HW_temp25_increment_ppb[dat$HW == 1 & dat$temp >= 25] <-
  2 + 2 * (dat$temp[dat$HW == 1 & dat$temp >= 25] - 25)


############################################################
## 4.5. Counterfactual and observed ozone
############################################################
...
dat$O3_nohw <- dat$O3_base +
  dat$o3_temp25_increment_ppb

dat$O3_nohw <- pmax(dat$O3_nohw, 10)
...
##   3) HW-only increment
##   4) HW x temp >= 25 C increment
dat$O3 <- dat$O3_base +
  dat$o3_temp25_increment_ppb +
  dat$o3_HW_increment_ppb +
  dat$o3_HW_temp25_increment_ppb

dat$O3 <- pmax(dat$O3, 10)


############################################################
## 4.6. Heat-wave-mediated ozone contrast
############################################################

## This is the O3 contrast used later through:
##   logO3 - logO3_nohw
##
## It should be zero for HW == 0 days, including HW == 0
## and temp >= 25 C days.
dat$heat_o3_increment_ppb <- dat$O3 - dat$O3_nohw


############################################################
## 4.7. Log ozone variables
############################################################

## Log ozone variables used for the TE component calculations
dat$logO3_nohw <- log(dat$O3_nohw)
dat$logO3 <- log(dat$O3)


############################################################
## 4.8. Reference ozone level
############################################################
...
############################################################
## 4.9. Prediction uncertainty for log ozone
############################################################

## Prediction uncertainty for log ozone.
## Synthetic constant value for the example.
dat$o3_sigma <- 1
...
write.csv(
  sim_city,
  file = "FOLDER_NAME/Data/sim_city_daily.csv",
  row.names = FALSE
)
...
print(summary(sim_city[, c("temp", "O3_nohw", "O3", "logO3_nohw", "logO3", "death")]))
